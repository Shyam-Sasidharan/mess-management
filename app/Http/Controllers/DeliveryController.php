...
{
    public function index(Request $request, DeliveryService $service, SubscriptionDateService $dates): View
    {
        $date = Carbon::parse($request->input('date', today()));
        $service->generateForDate($date);
        $isHoliday = $dates->isHoliday($date);
        $deliveries = Delivery::with(['customer', 'subscription'])->whereDate('delivery_date', $date)
            ->when($isHoliday, fn ($q) => $q->whereRaw('1 = 0'))
        ...
        return view('deliveries.index', compact('deliveries', 'date', 'isHoliday'));
    }
