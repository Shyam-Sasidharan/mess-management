    ...
    <div class="col-lg-7"><div class="card h-100"><div class="card-body">
        ...
        @if($currentSubscription)
            <div class="row g-3">
                ...
                <div class="col-sm-4"><div class="small text-muted">Payment</div><div class="fw-bold fs-5">{{ \App\Models\Setting::value('currency', '₹') }}{{ number_format($currentSubscription->amount, 2) }}</div>@if($subscriptionDueEligibility[$currentSubscription->id])<span class="badge status-{{ $currentSubscription->payment_status }}">{{ ucfirst($currentSubscription->payment_status) }}</span> <span class="small">Due {{ number_format($currentSubscription->outstanding_amount, 2) }}</span>@else<span class="badge bg-light text-dark">Not due</span> <span class="small text-muted">No food delivered in this renewal</span>@endif</div>
            </div>
        @else
...
<div class="card mb-4"><div class="card-body">
    ...
    @foreach($customer->subscriptions->sortByDesc('id') as $subscription)
        ...
        <tr><td class="fw-semibold">{{ $subscription->subscription_no }}</td><td>{{ $subscription->start_date->format('d M Y') }} — {{ ($subscription->original_end_date ?? $subscription->end_date)->format('d M Y') }}</td><td>{{ $subscription->end_date->format('d M Y') }}</td><td>{{ $subscription->holiday_compensation_days + $subscription->meal_hold_compensation_days }} days</td><td>{{ $subscription->meal_names }}</td><td class="text-end">{{ number_format($subscription->amount, 2) }}</td><td class="text-end text-success">{{ number_format($subscriptionPaid, 2) }}</td><td class="text-end fw-semibold {{ $subscriptionDueEligibility[$subscription->id] && $subscriptionBalance > 0 ? 'text-danger' : 'text-success' }}">{{ $subscriptionDueEligibility[$subscription->id] ? number_format($subscriptionBalance, 2) : '—' }}</td><td>@if($subscriptionDueEligibility[$subscription->id])<span class="badge status-{{ $subscription->payment_status }}">{{ ucfirst($subscription->payment_status) }}</span>@else<span class="badge bg-light text-dark">Not due</span>@endif</td><td><span class="badge status-{{ $subscription->status }}">{{ ucfirst($subscription->status) }}</span></td></tr>
    @endforeach
    </tbody></table></div>
</div></div>
